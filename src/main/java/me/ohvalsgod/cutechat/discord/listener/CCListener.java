            ...
            if (!event.getAuthor().isBot()) {
                ...
                for (Player player : Bukkit.getOnlinePlayers()) {
                    ...
                    if (data.getSettings().getChatStatus() == ChatStatus.BOTH || data.getSettings().getChatStatus()== ChatStatus.DISCORD_ONLY) {
                        player.sendMessage(color + CuteCHAT.getInstance().getPlayerDataHandler().getPlayerNameFromDiscordId(event.getAuthor().getId(), event.getAuthor().getName()) + ChatColor.WHITE + ": " + message);
                    }
                }
            }
